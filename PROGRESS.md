...
## 2026-07-20 — /letters 배경음악: 쇼팽 자장가 Op.57 (CC0)
- `소리 켜기` 가 이제 합성 저음만이 아니라 피아노 한 곡을 튼다. Musopen 녹음(archive.org, CC0 — 크레딧 의무 없음) 4:16 / 4.5MB.
- 통째 디코딩 대신 `<audio>` → MediaElementSource 로 스트리밍(메모리·첫 소리 대기 절감). 켜기 전엔 파일을 받지도 않고, 끄면 1.4초 뒤 정지.
- 저음 앰비언트는 곡과 같은 D♭ 로 조율(A2 → D♭2·A♭2)하고 0.65 → 0.3 으로 낮춰 피아노 아래 깔리게. 종이·소인 효과음은 유지.
- 검증: 켬 → 재생 중(loop, ctx running) / 끔 → pause / 재개 확인, mp3 206 스트리밍, 콘솔 에러 0.

## 2026-07-20 — /letters 첫 실제 일기 등록 (2026-08-11)
- 플레이스홀더 전면 제거: 더미 3세트·더미 이미지 5장·`scripts/gen-letter-dummies.py`·더미 blur 맵 삭제, `LETTERS = REAL` 로 단순화.
- 실제 노트 스캔 1장 등록. 폰 사진(4032×3024, 이불 배경) → 90° 회전 → 네 변 직선 피팅으로 원근 보정 →
  배경 완전 제거 → 조명 평탄화·크림 화이트밸런스·언샤프 → 잘려 나간 윗여백 복원. 전처리 스크립트는 일회성이라 레포에 남기지 않음.
- 손글씨 가독성 때문에 `sync-letters.ts` 의 MAX_EDGE 1600 → 2000. transcript(전문)·나의 답장 작성 완료.
- 검증: build OK, 게이트 통과 후 모바일·데스크톱 스크린샷에서 카드·전문·답장·라이트박스 정상, 콘솔 에러 0.
...
- 캡쳐 파이프라인: `npm run sync:letters` — 파일 스캔 → webp(해시 파일명으로 URL 추측 차단) → blur → letters.ts AUTO 구간 생성.
  reply·transcript·alt 는 보존된다. 원본은 `_src/`(gitignore). 더미/플레이스홀더는 전부 제거됨 — 실제 편지 대기 중.
